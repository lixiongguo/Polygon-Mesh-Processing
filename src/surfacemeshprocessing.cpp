...
void SurfaceMeshProcessing::CreateActions(void)
{
	...
	actSmooth->setStatusTip(tr("Shpw smooth"));
	connect(actSmooth, SIGNAL(triggered()), viewer, SLOT(ShowSmooth()));

	actParamAverage = new QAction(tr("Average Weighted"), this);
	actParamAverage->setStatusTip(tr("Neighbor weighted Averaged"));
	connect(actParamAverage, SIGNAL(triggered()), viewer, SLOT(TutteParam_AverageWeight()));

	actParamFloater = new QAction(tr("Floater Weighted"), this);
	actParamFloater->setStatusTip(tr("Neighbor weighted Floater"));
	connect(actParamFloater, SIGNAL(triggered()), viewer, SLOT(TutteParam_FloaterWeight()));

	actLSCMParam = new QAction(tr("LSCM Parm"), this);
	actLSCMParam->setStatusTip(tr("LSCM Param"));
	connect(actLSCMParam, SIGNAL(triggered()), viewer, SLOT(LSCMParam()));
...
void SurfaceMeshProcessing::CreateMenus(void)
{
	...
	menuTools->addAction(actLSCMParam);
